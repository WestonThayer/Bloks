...
IFont IListStyle::GetNthLevelLabelFont( ATETextDOM::Int32 index)
{
    FontRef ret = 0;
    ATEErr error = sListStyle->GetNthLevelLabelFont(fListStyle, index, &ret );
    if(error) Throw_ATE_Exception(error);
    return IFont(ret);
}

void IListStyle::GetNthLevelSequenceGenerator(ATETextDOM::Int32   index,
...
{
    ATEErr error = sListStyle->GetNthLevelSequenceGenerator(fListStyle, index, sequenceGeneratorType, prefix, postfix, bullet );
    if(error) Throw_ATE_Exception(error);
}
...
void IListStyle::SetNthLevelLabelFont(ATETextDOM::Int32 index, IFont font)
{
    ATEErr error = sListStyle->SetNthLevelLabelFont(fListStyle, index, font.GetRef());
    if(error) Throw_ATE_Exception(error);
}
...
void IListStyle::InsertLevelAt( ATETextDOM::Int32              index,
                                ...
                                const ATETextDOM::Unicode*     bullet)
{
    ATEErr error = sListStyle->InsertLevelAt(fListStyle, index, units, textIndent, labelIndent, labelAlignment, font.GetRef() , sequenceGeneratorTag, prefix, postfix, bullet );
    if(error) Throw_ATE_Exception(error);
}
...
ATETextDOM::Int32 IListStyleSet::Add( IListStyle ListStyle)
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyleSet->Add(fListStyleSet, ListStyle.GetRef(), &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}
