...
*/
	...
	*/
	...
	/** Create a stack-based Zstandard Data Filter. Normally linked to other filters in the chain
		to provide zstd compression/decompression
		#AIDataFitlerSuite::NewZStdDataFilter()

		//Write compressed content to a file
		auto dataFilter =	ai::CreateFileDataFilter(filePath, IDataFilter::Mode::kWrite, 0, 0,
									ai::CreateZStdDataFilter(IDataFilter::Mode::kWrite)
								);
		...
		//Read uncompressed content from a file
		auto dataFilter =	ai::CreateFileDataFilter(filePath, IDataFilter::Mode::kRead, 0, 0,
									ai::CreateZStdDataFilter(IDataFilter::Mode::kRead)
								);
		...
		nextFilter [in] Next filter in the chain. Equivalent to calling Link() after filter creation.
	*/
	std::unique_ptr<IDataFilter> CreateZStdDataFilter(
		IDataFilter::Mode filterMode,
		std::unique_ptr<IDataFilter> nextFilter = nullptr
	);

	/** Create a stack-based Async Data Filter.Takes a asyncFilter and makes it run on a background thread.
	async filter can be normally linked to other filters in the chain. This filter takes the data from the next item in the chain
	and passes it to asyncFilter on background thread.
	#AIDataFitlerSuite::NewAsyncDataFilter()

	Currently only supports Read Mode.
	asyncFilter [in] the filters that will be running on a background thread. this filter should not be linked to any other filter
	prefetchSize [in] size of a buffer typically fetched from the next filter(next of the filter returned by this API).
	outputSize   [in] size of a buffer typically asked from this filter.
	nextFilter [in] Next filter in the chain. Equivalent to calling Link() after filter creation.
*/
	std::unique_ptr<IDataFilter> CreateAsyncDataFilter(
		IDataFilter::Mode filterMode, 
