...
#include <cmath>
	...
	#include "AutoSuite.h"
	use_suite_required(AIDictionary)
	use_suite_required(AIArt)
	use_suite_required(AIDocument)
	use_suite_required(AIArray)
	use_suite_required(AIEntry)
	use_suite_required(AIDictionaryIterator)
...
#if AI_AUTO_SUITE_AVAILABLE
	use_suite_required(AIPathStyle)
#elif defined(_IAIDICTIONARY_SUITE_INCLUDE_H)
...
namespace ai
{
	...
	public:
        ...
        {
            return sAIDictionary->Key(keyString);
        }
    ...
    Dictionary::KeyType Dictionary::GetKey(const char* keyString)  const
    {
        return pImpl->GetKey(keyString);
    }
	...
	ai::Dictionary::iterator Dictionary::find(KeyType key) const
	{
