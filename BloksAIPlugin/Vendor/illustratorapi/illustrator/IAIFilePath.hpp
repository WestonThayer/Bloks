...
#endif
#ifdef LINUX_ENV
const ai::uint32 MaxPath = PATH_MAX;
#endif
...
class FilePath
{
	...
	explicit FilePath(const ai::UnicodeString &path, bool expandName = true,
                    bool dontStripTrailingSpace = false);
	...
	*/
	...
	/**	Sets this file path from a Unicode string containing a path native to
		Windows, Unix, or Mac OS, or a URL.
			...
			@return Nothing.
		*/
	void Set(const ai::UnicodeString &path, bool expandName = true, bool dontStripTrailingSpace = false);
	...
	/** Retrieves the path component delimiter for the current platform.
		...
		*/
	static char GetDelimiter() AINOTHROW;
