...
#ifndef __AI140Font__
#define __AI140Font__
...
#define kAI140FontSuite			"AI Font Suite"
#define kAIFontSuiteVersion3	AIAPI_VERSION(3)
#define kAI140FontSuiteVersion	kAIFontSuiteVersion3
#define kAI140FontVersion		kAI140FontSuiteVersion
...
struct AI140FontSuite {
	...
	/** Finds an Illustrator font matching a Mac OS system font.
			@param familyID The Mac OS font-family identifier.
			@param styleCode The Mac OS variant identifier
			@param result [out] A buffer in which to return the font key.
		*/
	...
	/** Retrieves the Mac OS system font for an Illustrator font. If the given font does not
		correspond to a system font, gets the default system font.
			@param fontKey The font key.
			@param familyResult [out] A buffer in which to return the Mac OS font-family identifier.
			@param styleResult [out] A buffer in which to return the Mac OS variant identifier
		 */
	...
	/** Retrieves a glyph set, which contains information about alternate
		glyphs within an OpenType font.
			...
			@param otFeature When zero, gets information about all glyphs. Otherwise,
				specify the feature of interest, as a 4-byte character sequence (in a \c long)
	...
	/** Retrieves all OpenType features supported by a particular font.
			...
			@param featureList [in, out] An array of OpenType features of length \c featureCount.
				Each value is a 4-byte character sequence (in a \c long) representing
	...
	/** Reports whether there are any glyphs for a given Open Type feature in a given font, or retrieves
		...
		(Note that this function returns a numeric value, not an error code.)
			@param theFeature The feature, a 4-byte character sequence (in a \c long) representing
				an OpenType feature tag.
			@param fontKey The font key.
	...
	/** Reports how many files constitute a font. For example, a Windows Type 1 font has a PFB
		...
		*/
	AIAPI AIErr (*GetFontFilePathCount) ( AIFontKey fontKey, ASUInt32& count );
	...
	/** Retrieves a file path associated with a font. Use with \c #GetFontFilePathCount()
		 ...
		 */
	AIAPI AIErr (*GetNthFilePath) ( AIFontKey fontKey, ASUInt32 n, ai::FilePath& filePath );
	...
	/** Gets the number of ATC components for a font. If the font is fauxed,
		...
		is an ATC font, using \c #GetFontInfo().
			...
			*/
	AIAPI AIErr (*GetNumATCComponents) ( AIFontKey fontKey, ASUInt32& count );
	...
	/** Retrieves a component for an ATC font, by position index. Use with
		...
		*/
	AIAPI AIErr (*GetNthATCComponent) ( AIFontKey fontKey, ASUInt32 n, AIFontKey& componentFontKey );
